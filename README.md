...
The first-party [`flutter_watchos`](packages/flutter_watchos) package adds the watch-specific APIs the framework doesn't cover: Digital Crown scrolling and raw input, Taptic Engine haptics, device info, and the system-clock toggle. A plugin that only implements iOS or macOS needs a watchOS implementation added under this key — see [Using and writing watchOS plugins](doc/plugins.md), and [`plugin port`](doc/plugin-porting.md) to scaffold one from the existing iOS implementation. Ready-made `*_watchos` packages live in [flutterwatch/plugins](https://github.com/flutterwatch/plugins).
