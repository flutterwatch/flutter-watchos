...
''';
  ...
  Future<(String, Set<String>, Set<String>)?> _buildPluginStaticArchive(
    ...
    final sources = <({String plugin, String root, String path})>[];
    ...
    for (final plugin in plugins) {
      ...
      for (final FileSystemEntity entity in pluginDir.listSync(recursive: true)) {
        ...
        if (p.endsWith('.m') || p.endsWith('.mm') || p.endsWith('.c')) {
          if (!hasExternalDeps) {
            sources.add((plugin: plugin.name, root: pluginDir.path, path: p));
          }
    ...
    for (final src in sources) {
      ...
      if (r.exitCode != 0) {
        globals.logger.printError(
          'Compiling watchOS plugin source ${src.path} failed:\n${r.stderr}',
        );
  ...
  /// The lines themselves come from [nativeLinkFlagsXcconfig].
  void _appendNativeLinkFlags(
    ...
    // up via its base configuration.
    watchosProjectDir
        .childDirectory('Flutter')
        .childFile('Generated.xcconfig')
        .writeAsStringSync(
          nativeLinkFlagsXcconfig(
            sdkName: buildInfo.sdkName,
            hostArchive: hostArchive,
            pluginArchive: pluginArchive,
          ),
